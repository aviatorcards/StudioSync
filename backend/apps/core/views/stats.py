...
class DashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        if user.role == 'admin':
            ...
            # 5. Unpaid Invoices
            from apps.billing.models import Invoice
            unpaid_qs = Invoice.objects.filter(studio__owner=user, status__in=['sent', 'overdue', 'partial'])
        ...
        # STUDENT VIEW
            ...
            # 1. Next Lesson
            next_lesson_obj = Lesson.objects.filter(
                student=student,
            ...
            if student.bands.exists():
                unpaid_qs = Invoice.objects.filter(band__in=student.bands.all(), status__in=['sent', 'overdue', 'partial'])
                unpaid_val = sum(inv.balance_due for inv in unpaid_qs)
        ...
        qs = Lesson.objects.select_related('student__user', 'teacher__user').filter(
            status__in=['completed', 'cancelled', 'scheduled']
        ).order_by('-updated_at')
