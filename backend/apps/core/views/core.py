...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def assign_to_band(self, request, pk=None):
        ...
        if user.role != 'student' or not hasattr(user, 'student_profile'):
            return Response({"detail": "Only students can be assigned to bands"}, status=status.HTTP_400_BAD_REQUEST)
        
        band_id = request.data.get('band_id')
        if not band_id:
            return Response({"band_id": ["This field is required."]}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def remove_from_band(self, request, pk=None):
        ...
        user = self.get_object()
        if user.role != 'student' or not hasattr(user, 'student_profile'):
            return Response({"detail": "Only students are in bands"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def link_family(self, request, pk=None):
        ...
        if not parent_id:
            return Response({"parent_id": ["This field is required."]}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if target_user.role != 'student' or not hasattr(target_user, 'student_profile'):
            return Response({"detail": "Relationships must target a student profile"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def create(self, request):
        ...
        try:
            ...
            if studio:
                if role == 'student':
                    Student.objects.create(user=user, studio=studio, instrument='Piano') # Default instrument
                elif role == 'teacher':
    ...
    def send_test_email(self, request):
        ...
        from apps.core.email_utils import send_test_email, get_email_settings
        ...
        if not all([smtp_host, smtp_port, smtp_username, smtp_password, smtp_from_email]):
             ...
             return Response({"detail": "Missing SMTP configurations"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def list_all(self, request):
        ...
        if not studio:
            ...
            parent_users = User.objects.filter(
                ...
                secondary_parent_families__studio=studio
            )
...
class StudioViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get'], permission_classes=[permissions.AllowAny], url_path='by-subdomain/(?P<subdomain>[^/.]+)')
    ...
    @action(detail=False, methods=['get'], permission_classes=[permissions.AllowAny], url_path='by-subdomain/(?P<subdomain>[^/.]+)/teachers')
    ...
    def get_queryset(self):
        ...
        if self.request.user.role == 'admin':
            queryset = Student.objects.filter(studio__owner=self.request.user, user__role='student')
             ...
             if hasattr(self.request.user, 'teacher_profile'):
                 queryset = Student.objects.filter(studio=self.request.user.teacher_profile.studio, user__role='student')
