...
def generate_teacher_calendar(teacher):
    ...
    for lesson in lessons:
        ...
        # Basic info
        event.add('summary', f'🎵 {lesson.student.user.get_full_name()} - {lesson.student.instrument}')
...
def generate_student_calendar(student):
    ...
    for lesson in lessons:
        ...
        # Basic info
        event.add('summary', f'🎹 {student.instrument} Lesson with {lesson.teacher.user.get_full_name()}')
        event.add('dtstart', lesson.scheduled_start)
        event.add('dtend', lesson.scheduled_end)
        event.add('dtstamp', timezone.now())
...
def generate_studio_calendar(studio):
    ...
    lessons = Lesson.objects.filter(
        teacher__studio=studio,
        scheduled_start__gte=start_date,
        scheduled_start__lte=end_date
    ).exclude(status='cancelled').select_related('teacher__user', 'student__user', 'student')
    
    for lesson in lessons:
        event = Event()
        
        # Basic info
        event.add('summary', f'{lesson.teacher.user.get_full_name()} → {lesson.student.user.get_full_name()}')
        event.add('dtstart', lesson.scheduled_start)
        event.add('dtend', lesson.scheduled_end)
        event.add('dtstamp', timezone.now())
